...
sub ifaceTest
{
  ...
  foreach my $iface  (keys %fakeIfaces) {
    ...
    my @actualAddress = sort ( EBox::NetWrappers::iface_address($iface) );
    eq_or_diff \@actualAddress, \@expectedAddress, "Testing result of iface_address($iface)";

    eq_or_diff EBox::NetWrappers::iface_address_with_netmask($iface), $fakeIfaces{$iface}->{address}, "Testing result of iface_address_with_netmask($iface)"
