...
package EBox::CGI::OpenVPN::RemoveAdvertisedNet;
...
sub actuate
{
    ...
    $server->removeAdvertisedNet($network);

    $self->setMsg(__x('Server no longer grants access to {network}', network => $network));
}


sub requiredParameters
{
    return [qw(network name )]; 
}
