...
package EBox::CGI::Printers::ModelUI;
...
sub new {
	my $class = shift;
	my $self = $class->SUPER::new('title'    => __('Add a new printer (IV)'),
				      'template' => 'printers/models.mas',
				      @_);
...
sub _process($) {
	...
	push(@array,  'models' => $printers->manufacturerModels($id));
	$self->{params} = \@array;
