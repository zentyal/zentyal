...
package EBox::CGI::Printers::NetworkPrinterUI;
...
sub new {
	my $class = shift;
	my $self = $class->SUPER::new('title'    => __('Network printers'),
				      'template' => 'printers/network.mas',
				      @_);
...
sub _process($) {
	...
	my @array;
        ...
        push (@array, 'button_name' => 'networkconfui');
	$self->{params} = \@array;
}
