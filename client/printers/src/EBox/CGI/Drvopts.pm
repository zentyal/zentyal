...
package EBox::CGI::Printers::Drvopts;
...
sub _process($) {
	...
	my @filter = @{$printers->driverArgs($id)};
	my $args;
	foreach my $opt (@filter) {
			my $parm = $self->param($opt);;
			$args->{$opt} = $parm if defined($parm);
	}
	$printers->setDriverOptions($id, $args);
