...
package EBox::CGI::Printers::ParallelPrinterUI;
...
sub new {
	my $class = shift;
	my $self = $class->SUPER::new('title'    => __('Parallel printers'),
				      'template' => 'printers/parallel.mas',
				      @_);
...
sub _process($) {
	...
	push (@array, 'button_name' => 'parallelconfui');
	$self->{params} = \@array;
}
