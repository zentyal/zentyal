/*
 * $Id: vscan-trend.c,v 1.53 2003/06/25 10:19:16 mx2002 Exp $
 * 
 * virusscanning VFS module for samba.  Log infected files via syslog
 * facility and block access using Trophie.
 *
...
#endif
{
	...
	/* scan only file, do nothing */	
		...
		/* scan only file, do nothing */
                rv = vscan_trend_scanfile(sockfd, filepath, client_ip);
                vscan_trend_end(sockfd);
		if ( rv == 1 ) {
			/* virus was found */
#if (SMB_VFS_INTERFACE_VERSION >= 6)
