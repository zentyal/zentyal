/*
 * $Id: vscan-sophos.c,v 1.54 2003/06/25 10:19:16 mx2002 Exp $
 * 
 * virusscanning VFS module for samba.  Log infected files via syslog
 * facility and block access using Sophie.
 *
...
#define VSCAN_MODULE_STR "vscan-sophos"
...
fstring	sophie_socket_name;	/* name of Sophie socket */
...
static BOOL do_parameter(const char *param, const char *value)
{
	...
	} else if ( StrCaseCmp("sophie socket name", param) == 0) {
		fstrcpy(sophie_socket_name, value);
		DEBUG(3, ("sophie socket name is %s\n", sophie_socket_name));
...
#endif
{
	...
	/* name of Sophie socket */
	fstrcpy(sophie_socket_name, SOPHIE_SOCKET_NAME);
...
#endif
{
	...
	else  
	{
		...
		sockfd = vscan_sophos_init();
			...
			/* an error occured - can not communicate to daemon - deny access */
                                vscan_syslog("ERROR: can not communicate to Sophie - access to file %s denied", fname);
				vscan_sophos_end(sockfd);
...
#endif
{
	...
	sockfd = vscan_sophos_init();
		...
		/* scan only file, do nothing */
                rv = vscan_sophos_scanfile(sockfd, filepath, client_ip);
                vscan_sophos_end(sockfd);
		if ( rv == 1 ) {
			/* virus was found */
#if (SMB_VFS_INTERFACE_VERSION >= 6)
