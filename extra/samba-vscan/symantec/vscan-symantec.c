/* 
 * $Id: vscan-symantec.c,v 1.42 2003/06/25 10:19:16 mx2002 Exp $
 *
 * virusscanning VFS module for samba.  Log infected files via syslog
 * facility and block access using Symantec CarrierScan .
 *
...
#define VSCAN_MODULE_STR "vscan-symantec"
...
fstring cs_ip_port;		/* IP and port CarrierScan listens on */
...
static BOOL do_parameter(const char *param, const char *value)
{
		...
		*/
        ...
        } else if ( StrCaseCmp("cs ip port", param) == 0 ) {
                fstrcpy(cs_ip_port, value);
                DEBUG(3, ("Symantec CarrierScan IP:port is: %s\n", cs_ip_port));
        } else
...
#endif
{
	...
	send_warning_message = VSCAN_SEND_WARNING_MESSAGE;
        ...
        /* set default value for Symantec CarrierScan IP:port */
        fstrcpy(cs_ip_port, VSCAN_CS_IP_PORT);
...
#endif
{
	...
	else  
	{
		...
		vscan_symantec_init();
			...
			/* scan file */
			retval = vscan_symantec_scanfile(filepath, client_ip);
			if ( retval == -2 && deny_access_on_minor_error ) {
				...
				return -1;
                	        ...
                	        /* an error occured - can not communicate to daemon - deny access */
                        	vscan_syslog("ERROR: can not communicate to CarrierScan server - access to file %s denied", fname);
