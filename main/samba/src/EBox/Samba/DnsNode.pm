...
package EBox::Samba::DnsNode;

use EBox::Exceptions::MissingArgument;
use EBox::Samba::DnsRecordSOA;
use EBox::Samba::DnsRecordSRV;
use EBox::Samba::DnsRecordTXT;
use EBox::Samba::DnsRecordNS;
use EBox::Samba::DnsRecordMX;
use EBox::Samba::DnsRecordA;
...
sub _decodeDnsRecord
{
    ...
    my ($dataLength, # 2 bytes | Length (bytes) of the data field
        ...
        $data) = unpack ('S S C C S L L L L a*', $blob);
    ...
    return new EBox::Samba::DnsRecordSOA(data => $data) if ($type == DNS_TYPE_SOA);
    return new EBox::Samba::DnsRecordA(data => $data)   if ($type == DNS_TYPE_A);
    return new EBox::Samba::DnsRecordSRV(data => $data) if ($type == DNS_TYPE_SRV);
    return new EBox::Samba::DnsRecordTXT(data => $data) if ($type == DNS_TYPE_TXT);
    return new EBox::Samba::DnsRecordNS(data => $data)  if ($type == DNS_TYPE_NS);
    return new EBox::Samba::DnsRecordMX(data => $data)  if ($type == DNS_TYPE_MX);
