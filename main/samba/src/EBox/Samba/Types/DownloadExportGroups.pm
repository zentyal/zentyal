...
# Class: EBox::Samba::Types::DownloadExportGroups;
#
use strict;
use warnings;

package EBox::Samba::Types::DownloadExportGroups;
...
sub value
{
    ...
    if (-f '/var/lib/zentyal/tmp/.groups_exporter-running') {
        return 'in-progress';
    } else {
        my $csv = '/tmp/groups-export.csv';
        if (-f $csv) {
            return 'available';
        } else {
            return 'noreport';
...
sub HTMLViewer
{
    return '/samba/downloadExportGroupsViewer.mas';
}
