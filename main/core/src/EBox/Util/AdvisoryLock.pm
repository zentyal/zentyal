...
# Class: EBox::Util::AdvisoryLock
#
#   Special class to lock using flock in an advisory way.
#

use strict;
use warnings;

package EBox::Util::AdvisoryLock;
...
#
#     blockingTime - Int if we want the lock exclusively, use this value
...
sub lock
{
    my ($resource, $blocking, $blockingTime) = @_;

    $blockingTime = 0 unless (defined($blockingTime));
    ...
    if ($blocking) {
        if ($blockingTime > 0) {
            eval {
                ...
                ## Cancel the alarm if lock is got within $blockingTime sec.
                alarm(0);
            }; if ($@) {
                if ($@ eq "Timed out\n") {
...
{
    my ($resource) = @_;
    return EBox::Config::tmp .  $resource . ".adv.lock";
}
