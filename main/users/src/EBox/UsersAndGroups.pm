...
use constant KDC_CONF_FILE  => '/etc/krb5kdc/kdc.conf';
use constant KERBEROS_SECRET_FILE => '/etc/krb5kdc/service.keyfile';
...
sub usedFiles
{
    ...
    push(@files,
        ...
        {
            'file' => KERBEROS_SECRET_FILE,
            'reason' => __('To stash the LDAP admin password generated by ' .
                'Zentyal and allow kerberos daemon to access LDAP.'),
            'module' => 'users'
        },
    );
...
sub initialSetup
{
    ...
    unless ($version) {
        ...
        $fw->addInternalService(
                'name' => 'kdc',
                'printableName' => 'KDC',
                'description' => __('Kerberos Key Distribution Center'),
                'protocol' => 'tcp/udp',
                'sourcePort' => 'any',
                'destinationPort' => 88,
                'target' => 'accept',
        ...
        $fw->addInternalService(
                ...
                'description' => __('Kerberos administration and password change'),
                'protocol' => 'tcp/udp',
                'sourcePort' => 'any',
                'destinationPort' => 749,
                'target' => 'accept',
...
sub enableActions
{
    ...
    try {
        ...
        $self->createKerberosDB($dn, $realm, $password);
    } otherwise {
        my $error = shift;
...
sub createKerberosDB
{
    my ($self, $dn, $realm, $ldapPassword) = @_;

    EBox::debug('Loading kerberos database');
    my $kerberosMasterPassword = $self->_genPassword(EBox::Config::conf() . "$realm.KerberosMaster.passwd");
    my $cmd = "kdb5_ldap_util " .
              "-D cn=zentyal,$dn " .
              "-w '$ldapPassword' " .
              "create -subtrees $dn " .
              "-P '$kerberosMasterPassword' " .
              "-s -H ldapi:/// -r $realm";
    EBox::debug("Executing '$cmd'");
    my @output = @{EBox::Sudo::root($cmd)};
    EBox::debug("Command output: @output");

    EBox::debug("Creating file with stashed password");
    my $content = "cn=zentyal,$dn#{HEX}" . unpack ('H*', $ldapPassword);
    my @cmds = ();
    push (@cmds, "echo '$content' > " . KERBEROS_SECRET_FILE);
    push (@cmds, 'chmod 400 ' . KERBEROS_SECRET_FILE);
    EBox::Sudo::root(@cmds);
}
...
sub _setConf
{
    ...
    my $defaultDomain = lc ($self->kerberosDefaultRealm());
    @array = ();
    push (@array, 'defaultRealm' => $self->kerberosDefaultRealm());
    push (@array, 'defaultDomain' => $defaultDomain);
    push (@array, 'kdcs' => $self->kerberosKDCs());
    push (@array, 'adminServer' => $self->kerberosAdminServer());
    push (@array, 'ldapDN' => $dn);
    push (@array, 'adminDn' => $self->ldap->rootDn());
    push (@array, 'adminRoDn' => $self->ldap->rootDn()); # TODO implement rootRoDn in Ldap.pm
    $self->writeConfFile(KRB5_CONF_FILE, 'users/krb5.conf.mas',
            \@array);
