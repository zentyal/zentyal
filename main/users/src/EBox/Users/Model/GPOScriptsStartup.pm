...
#
# Class: EBox::Users::Model::GPOScriptsStartup
#
package EBox::Users::Model::GPOScriptsStartup;

use base 'EBox::Users::Model::GPOScripts';

use EBox::Gettext;
use EBox::Users::GPO;
use EBox::Users::GPO::ScriptsComputer;
...
sub ids
{
    ...
    my $gpoDN = EBox::Users::GPOIdMapper::idToDn($gpoId);
    my $extension = new EBox::Users::GPO::ScriptsComputer(dn => $gpoDN);
...
sub addTypedRow
{
    ...
    $self->setMessage(__x('Startup script {x} added',
...
sub removeRow
{
    ...
    my ($type, $index) = split (/_/, $id);
    delete $data->{$type}->{Startup}->{$index};
    foreach my $i (sort keys %{$data->{$type}->{Startup}}) {
        my $newIdx = $i - 1;
        if ($i > $index) {
            $data->{$type}->{Startup}->{$newIdx} = $data->{$type}->{Startup}->{$i};
            delete $data->{$type}->{Startup}->{$i};
        }
    }
    $extension->write($data);

    $self->setMessage(__x('Startup script {x} removed', x => $name));
}
