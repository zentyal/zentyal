...
package EBox::AntiVirus::Types::Status;
...
sub value
{
    ...
    if (-f '/var/lib/zentyal/tmp/.clamscan-running') {
        return 'noreport';
    } else {
        my $log = '/usr/share/zentyal/www/antivirus.log';
        if (-f $log) {
            system("grep FOUND $log");
            return ($? == 0 ? 'infected' : 'clean');
        } else {
            return 'noreport';
        }
    }
...
sub HTMLViewer
{
    return '/antivirus/statusViewer.mas';
}
