...
sub _setIptablesRule # (gw, set)
{
    ...
    if ($set) {
        ...
        # when the traffic is generated by zentyal's web admin.
        # FIXME: With haproxy, webadmin may be sharing the port with other applications!!
        my $webAdminPort = EBox::Global->modInstance('webadmin')->usedHAProxySSLPort();
        my $rule = "/sbin/iptables -t mangle -A $chain "
             . "-p tcp --source-port $webAdminPort -j RETURN";
        EBox::Sudo::root($rule);
