...
livesOk(sub { $backupServ->makeRemoteBackup($name); }, 'Making remote backup');


livesOk( sub { $backupServ->makeRemoteBackup($name2); }, 'Making a second remote backup');
...
livesOk(sub { $backupServ->removeRemoteBackup($name) }, 'Removing remote backup');

livesOk(sub { $backupList = $backupServ->listRemoteBackups(); }, 'listing remote backups after removal');
...
livesOk(sub { $downloadedArchive = $backupServ->downloadRemoteBackup($name2); }, 'download a remote backup');
