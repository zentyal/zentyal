...
# Method: pullStatus
#
#    Get the results of a job in runnerd based on the unique identifier.
#
# Parameters:
#
#    jobId - Int the unique identifier for the job. <runJob> sets the jobId
#
# Returns:
#
#    Hash ref - containing the following keys:
#
#         - status : String with the following choices:
#
#               'queued'   - still not run
#               'noexist'  - this job id does not exist
#               'finished' - the job has finished.
#                            The result are in the other elements from the returned value.
#
#         - exitValue : Integer the exit value
#
#         - stdout : String which is the standard output generated by
#                    the script. The output format depends on the
#                    job. YAML is the most common format.
#
#         - stderr : String which the standard error generated by the
#                    script.
#
sub pullStatus
{
    ...
    my ($jobId) = @{$class->_gatherParams([qw(jobId)], @_)};

    my $retValue = _pullStatus($jobId);
    return $class->_soapResult($retValue);
}
...
sub _pullStatus
{
    ...
    my $jobDirPath = JOBS_DIR . $jobId;
    if ( -d $jobDirPath ) {
        my $incomingDirPath = INCOMING_DIR . $jobId;
        if ( -l $incomingDirPath ) {
            $retValue{status} = 'queued';
        } elsif (-f "$jobDirPath/exitValue" ) {
            $retValue{exitValue} = File::Slurp::read_file("$jobDirPath/exitValue");
            chomp($retValue{exitValue});
            $retValue{stdout} = File::Slurp::read_file("$jobDirPath/stdout");
            $retValue{stderr} = File::Slurp::read_file("$jobDirPath/stderr");
            $retValue{status} = 'finished';
        } else {
            EBox::warn("$jobId is not in the queue and it has never been run");
            $retValue{status} = 'queued';
        }
    }
